...
cdef tuple _c_single_source_shortest_distances(int source, list N, dict E, dict neighbors, tuple operator_names, int verbose):
	...
	Args:
		...
		verbose (int): print statements as it computed shortest distances.
...
#
#
def _cy_single_source_complete_paths(int source, list N, dict paths):
	"""
	The Cython wrapper for the C function `_c_single_source_shortest_paths`
	"""
	return _c_single_source_shortest_paths(source, N, paths)

cdef _c_single_source_shortest_paths(int source, list N, dict paths):
	"""
