...
class Dijkstra(object):
    ...
    def all_pairs_shortest_distances(self, kind='metric', n_jobs=1, engine='cython', verbose=0):
        ...
        if engine == 'python':
            ...
            poolresults = list(range(len(self.N)))
            for node in self.N:
                poolresults[node] = _py_single_source_shortest_distances(node, self.N, self.E, self.neighbours, operators, verbose)
    ...
    def get_shortest_distances(self, source=None, translate=True, format='dict'):
        """
    ...
    def get_shortest_paths(self, source=None, translate=True):
        """
...
if __name__ == '__main__':
    ...
    nx.set_edge_attributes(G, edgelist, 'weight')
    ...
    nx_all_complete_paths = dict(nx.all_pairs_dijkstra_path(G, weight='weight'))
